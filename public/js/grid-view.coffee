(->
	onDomLoad = ->
		...
		$(".grid-table").children("tbody").on("click", ".domain-title-cntnr .copy-to-clipboard", preventEvent).on("click", "td button.favourite", toggleFavourite).on("click", "td button.select", toggleSelect).on("click"
			, "td:not(#zero-alert)"
			, ->
				$this = $(this)
				$row = $this.closest("tr")
